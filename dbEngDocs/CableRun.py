...
def find_specificcable(id):
    """
    Short hand function to allow querying of specific cable by cable.id.
    :param id: 
    :return: 
    """
    return Cables.Cable.objects.find_specificcable(id).first()
...
class CableRunDefault(EmbeddedDocument):
    ...
    """
    # TODO: Add methods to allow update of those values.
    ...
    cableAllowParallelMulti = BooleanField(
        default=cableVar.default_cableAllowParallelMulti,
        required=True
    )
    ...
    def clean(self):
        ...
        # if not self.cableMaxParallelRuns:
        #     self.
    ...
    def setCableArmour(self, value):
        """
        Update cableArmour parameter.
        :return: 
        """

        self.cableArmour = value
        # self.save()
...
class CableRun(Document):
    ...
    # TODO: add method to all the addition of neutral and earth cables. → this includes adding the size earth to active in cable variables
    """
    Mongo Engine data checking class.
    ...
    def calc_description(self):
        ...
        :return:
        """
        # TODO: Allow for single core cables to be correctly required.
        x = self.cables
        i = dict()
        for each in x:
            key = each.id
        ...
        for cable in i:
            cablequery = find_specificcable(each.id)
            cabledescription = cablequery.description
            cablecorearrangement =cablequery.coreArrangement

            if i[cable] > 1:
                if cablecorearrangement == '1C':
                    description += ('{} * {}'.format(i[cable], cabledescription))
                else:
                    description += ('{} * {} '.format(i[cable], cabledescription))
            else:
                description += ('{} '.format(cabledescription))
    ...
    def calc_impedence(self):
        ...
        for each in self.cables:
            ...
            cable = find_specificcable(each.id)
    ...
    def check_cable_vd(self, cable, parallelcables=1):
        """
        ...
        volt_drop = EF.mvamvoltagedrop(
            ...
            float(cable.find_mvam()/parallelcables)
        )
    ...
    def check_CCC(self, cable=None, parallelcables=1):
        # TODO: Why do we allow cable=None here??
        """
        ...
        """
        try:
            if (cable.find_ccc(self.circuit.installMethod) * parallelcables * self.circuit.derating) > self.circuit.loadCurrent:
                return True
            else:
                return False
        except TypeError as e:
            if cable is None:
                if (self.circuit.CCC * parallelcables * self.circuit.derating) > self.circuit.loadCurrent:
                    return True
                else:
                    return False
            else:
    ...
    def find_cable(self, cable_query, maxparallelcables=4):
        """
        ...
        """
        parallel = 1
        while parallel <= maxparallelcables:
            for cable in cable_query:
                x = []
                if parallel == 1 or cable.coreArrangement == '1C' or (parallel > 1 and
                    self.default.cableAllowParallelMulti is True and cable.coreArrangement != '1C'):

                    if (self.check_CCC(cable, parallel) and self.check_cable_vd(cable, parallel)) is True:
                        self.clear_cable()
                        for each in range(parallel):
                            x.append(cable)
                        self.add_cable(x)
                        self.bulk_calc()
                        return True
    ...
    def find_cable_CCC(self, cable):
        ...
        :return: cable CCC
        """
        # TODO: Is this a double of the cable.find_ccc method?
        try:
    ...
    def query_cable(self, coretype='ACTIVE'):
        # TODO: Can this method be used to find the neutral and earth conductors at the same time as the ACTIVE cores?
        """
    ...
    def size_cablerun(self, **kwargs):
        ...
        if self.find_cable(cable_query) is False:
            ...
            print('{} : {}'.format(self.tag, self.description))
    ...
    def calcNeutral(self):
        # TODO:Do we allow the use of half size neutral? This should be a setting with default.
        """
        Calculate the neutral conductor for the cable run.
        1. Determine the CCC of the load.
        2. Determine neutral CCC using neutralSize * CCC of load.
        3. If neutral.CCC == active.CCC
        3a. Find neutral cable same size as active
        4. Else query cable type=Neutral CCC=neutralSize * load.CCC
        5. Add cable(s) to cable run
        6. recalc CSA and other circuit details.
        :return: 
        """
        CCC = self.circuit.loadCurrent
        if self.default.neutralSize == 1:

        for each in self.cables:
            cableCCC = self.find_cable_CCC(each)
            # cable = find_specificcable(each.id)
            # cableCCC = cable.find_ccc(self.circuit.installMethod)
            self.size_cablerun(coretype="NEUTRAL")
        pass
